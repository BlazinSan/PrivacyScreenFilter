...
static LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        if (id == ID_THEME_LIGHT || id == ID_THEME_DARK) {
            ...
            g_themeBrush = CreateSolidBrush(currentTheme == Theme::Dark ?
                RGB(40, 40, 40) : RGB(240, 240, 240));
            g_frameBrush = CreateSolidBrush(currentTheme == Theme::Dark ?
                RGB(30, 30, 30) : RGB(255, 255, 255));

            // Force redraw of all controls
            InvalidateRect(hwnd, NULL, TRUE);

            // Redraw ALL child windows with the new color
            ...
            g_settings.startMinimized = SendMessage(g_hStartMinimizedChk,
                BM_GETCHECK, 0, 0) == BST_CHECKED;
            SaveSettings();
        }
        else if (id == 103) {
            g_settings.startWithWindows = SendMessage(g_hStartWithWindowsChk,
                BM_GETCHECK, 0, 0) == BST_CHECKED;
            SetAutoStartWithWindows(g_settings.startWithWindows);
            SaveSettings();
        }
        else if (id == 104) {
            g_settings.enableOnStartup = SendMessage(g_hEnableOnStartupChk,
                BM_GETCHECK, 0, 0) == BST_CHECKED;
            SaveSettings();
        }
        return 0;
    ...
    case WM_CTLCOLORBTN: {
        ...
        // Check if this is for a shadow window
        if (GetProp(ctrlHwnd, L"IsShadow")) {
            COLORREF shadowColor = currentTheme == Theme::Dark ?
                SHADOW_COLOR_DARK : SHADOW_COLOR_LIGHT;
            static HBRUSH shadowBrush = NULL;
            if (shadowBrush) DeleteObject(shadowBrush);
            shadowBrush = CreateSolidBrush(shadowColor);
            return (LRESULT)shadowBrush;
        }

        // Control coloring with better contrast
        if (currentTheme == Theme::Dark) {
            SetTextColor(hdc, RGB(240, 240, 240));
            SetBkColor(hdc, RGB(20, 60, 80)); // Lighter blue for better contrast

            // Create and return a specific brush for dark mode controls
            static HBRUSH darkControlBrush = NULL;
            if (darkControlBrush) DeleteObject(darkControlBrush);
            darkControlBrush = CreateSolidBrush(RGB(20, 60, 80));
            return (LRESULT)darkControlBrush;
        }
        else {
            SetTextColor(hdc, RGB(0, 0, 0));
            SetBkColor(hdc, RGB(119, 201, 197));
            return (LRESULT)g_frameBrush;
        }
    }

    case WM_HOTKEY:
        if (wParam == 1) ToggleFilterVisibility();
        return 0;

    case WM_HSCROLL: {
        if ((HWND)lParam == g_hOpacitySlider) {
            g_settings.opacity = (int)SendMessage(g_hOpacitySlider, TBM_GETPOS, 0, 0);
            UpdateFilterWindow();
            SaveSettings();
        }
        else if ((HWND)lParam == g_hFadeWidthSlider) {
            g_settings.fadeWidth = (int)SendMessage(g_hFadeWidthSlider, TBM_GETPOS, 0, 0);
            UpdateFilterWindow();
            SaveSettings();
        }
        return 0;
    ...
    case WM_DESTROY: {
        ...
        if (g_themeBrush) DeleteObject(g_themeBrush);
        if (g_frameBrush) DeleteObject(g_frameBrush);
        if (g_hControlFont) DeleteObject(g_hControlFont);
...
static void AddControlHoverEffect(HWND hwnd) {
    // Brighten the control's background slightly
    LONG style = GetWindowLong(hwnd, GWL_STYLE);
    if ((style & BS_PUSHBUTTON) || (style & BS_DEFPUSHBUTTON)) {
        // For buttons, use a brighter color
        HBRUSH hoverBrush = CreateSolidBrush(g_settings.currentTheme == Theme::Dark ?
            RGB(20, 60, 80) : RGB(150, 220, 215));
        SetClassLongPtr(hwnd, GCLP_HBRBACKGROUND, (LONG_PTR)hoverBrush);
        InvalidateRect(hwnd, NULL, TRUE);
    }
}

static void RemoveControlHoverEffect(HWND hwnd) {
    LONG style = GetWindowLong(hwnd, GWL_STYLE);
    if ((style & BS_PUSHBUTTON) || (style & BS_DEFPUSHBUTTON)) {
...
// Update fonts with proper null checks
void UpdateFonts() {
    // Clean up previous fonts
    if (g_hControlFont) {
        DeleteObject(g_hControlFont);
        g_hControlFont = NULL;
    }
    ...
    // Create scaled fonts
    g_hControlFont = CreateFont(ScaleY(16), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");

    g_hHeaderFont = CreateFont(ScaleY(24), 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");

    // Update the font for all child controls
    if (g_hMainWnd && IsWindow(g_hMainWnd)) {
        HWND childHwnd = GetWindow(g_hMainWnd, GW_CHILD);
        while (childHwnd && IsWindow(childHwnd)) {
            DWORD ctrlId = GetDlgCtrlID(childHwnd);
            if (ctrlId == 0 && g_hHeaderFont) { // Header
                SendMessage(childHwnd, WM_SETFONT, (WPARAM)g_hHeaderFont, TRUE);
            }
            else if (g_hControlFont) {
                SendMessage(childHwnd, WM_SETFONT, (WPARAM)g_hControlFont, TRUE);
            }
...
void UpdateControlsLayout(HWND hwnd) {
    ...
    // Calculate scaled dimensions with proportions based on window width
    const int labelWidth = min(width / 4, ScaleX(150));
    const int controlWidth = min(width / 3, ScaleX(220));
    const int leftMargin = max(ScaleX(20), width / 20);
    const int topMargin = ScaleY(80);
    const int verticalSpacing = ScaleY(40);

    // Find and update other control positions
    int controlY = topMargin;

    HWND hLightTheme = GetDlgItem(hwnd, ID_THEME_LIGHT);
    HWND hDarkTheme = GetDlgItem(hwnd, ID_THEME_DARK);

    int radioWidth = ScaleX(100);
    int radioSpacing = ScaleX(20);

    SetWindowPos(hLightTheme, NULL, leftMargin, controlY,
        radioWidth, ScaleY(30), SWP_NOZORDER);
    SetWindowPos(hDarkTheme, NULL, leftMargin + radioWidth + radioSpacing, controlY,
        radioWidth, ScaleY(30), SWP_NOZORDER);

    controlY += verticalSpacing;

    // Opacity controls
    HWND hOpacityLabel = FindWindowEx(hwnd, NULL, L"STATIC", L"Opacity:");
    if (hOpacityLabel) {
        SetWindowPos(hOpacityLabel, NULL, leftMargin, controlY,
            labelWidth, ScaleY(30), SWP_NOZORDER);
    }

    if (g_hOpacitySlider) {
        SetWindowPos(g_hOpacitySlider, NULL, leftMargin + labelWidth + ScaleX(20), controlY,
            controlWidth, ScaleY(30), SWP_NOZORDER | SWP_SHOWWINDOW);
    }

    controlY += verticalSpacing;

    // Fade Width controls 
    HWND hFadeWidthLabel = FindWindowEx(hwnd, hOpacityLabel, L"STATIC", L"Fade Width:");
    if (hFadeWidthLabel) {
        SetWindowPos(hFadeWidthLabel, NULL, leftMargin, controlY,
            labelWidth, ScaleY(30), SWP_NOZORDER);
    }

    if (g_hFadeWidthSlider) {
        SetWindowPos(g_hFadeWidthSlider, NULL, leftMargin + labelWidth + ScaleX(20), controlY,
            controlWidth, ScaleY(30), SWP_NOZORDER | SWP_SHOWWINDOW);
    }
    ...
    // Hotkey controls
    HWND hHotkeyLabel = FindWindowEx(hwnd, hFadeWidthLabel, L"STATIC", L"Toggle Hotkey:");
    if (hHotkeyLabel) {
        SetWindowPos(hHotkeyLabel, NULL, leftMargin, controlY,
            labelWidth, ScaleY(30), SWP_NOZORDER);
    }

    if (g_hHotkeyCtrl) {
        SetWindowPos(g_hHotkeyCtrl, NULL, leftMargin + labelWidth + ScaleX(20), controlY,
            controlWidth, ScaleY(30), SWP_NOZORDER | SWP_SHOWWINDOW);
    }
    ...
    if (g_hEnableBtn) {
        SetWindowPos(g_hEnableBtn, NULL, leftMargin, controlY,
            min(width - 2 * leftMargin, controlWidth + labelWidth + ScaleX(30)), ScaleY(40),
            SWP_NOZORDER | SWP_SHOWWINDOW);
    }

    controlY += verticalSpacing + ScaleY(20);

    if (g_hStartMinimizedChk && g_hStartWithWindowsChk) {
        // Calculate consistent positions for both checkboxes
        int checkboxWidth = ScaleX(200);
        int checkboxHeight = ScaleY(30);

        // Position the Start minimized checkbox at a fixed left position
        SetWindowPos(g_hStartMinimizedChk, NULL, leftMargin, controlY,
            checkboxWidth, checkboxHeight, SWP_NOZORDER | SWP_SHOWWINDOW);

        // Position the Start with Windows checkbox at a fixed position to the right of the first one
        SetWindowPos(g_hStartWithWindowsChk, NULL, leftMargin + checkboxWidth + ScaleX(20), controlY,
            checkboxWidth, checkboxHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
    }
    ...
    if (g_hEnableOnStartupChk) {
        SetWindowPos(g_hEnableOnStartupChk, NULL, leftMargin, controlY,
            min(width - 2 * leftMargin, ScaleX(400)), ScaleY(30), SWP_NOZORDER | SWP_SHOWWINDOW);
    }

    // Make sure all controls are visible and with proper font
    HWND child = GetWindow(hwnd, GW_CHILD);
    while (child) {
        // Update the control's font if available
        if (g_hControlFont) {
            SendMessage(child, WM_SETFONT, (WPARAM)(g_hControlFont), TRUE);
        }

        // Show the control and force redraw
...
void DrawCustomTitleBar(HWND hwnd, HDC hdc) {
    ...
    // Calculate the title bar rect - extend it to cover the entire top edge
    RECT rcTitleBar = rcClient;
    rcTitleBar.bottom = ScaleY(30); // Height of titlebar

    // Fill title bar background with NO border at top
    COLORREF titleBarColor = g_settings.currentTheme == Theme::Dark ?
        RGB(61, 117, 117) : RGB(51, 102, 102);
    ...
    // Draw title text using GDI+
    Gdiplus::Graphics graphics(hdc);
    graphics.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
    graphics.SetTextRenderingHint(Gdiplus::TextRenderingHintClearTypeGridFit);

    Gdiplus::FontFamily fontFamily(L"Segoe UI");
    Gdiplus::Font font(&fontFamily, (REAL)ScaleY(16), Gdiplus::FontStyleBold, Gdiplus::UnitPixel);
    Gdiplus::RectF layoutRect(25.0f, 5.0f, static_cast<Gdiplus::REAL>(rcTitleBar.right - 20), 20.0f);
    Gdiplus::SolidBrush textBrush(g_settings.currentTheme == Theme::Dark ?
        Gdiplus::Color(240, 240, 240) : Gdiplus::Color(0, 0, 0));

    graphics.DrawString(title, -1, &font, layoutRect, nullptr, &textBrush);

    // Draw a separator line below titlebar (but NOT above it)
    HPEN pen = CreatePen(PS_SOLID, 1, g_settings.currentTheme == Theme::Dark ?
        RGB(60, 60, 60) : RGB(200, 200, 200));
    HPEN oldPen = (HPEN)SelectObject(hdc, pen);
    MoveToEx(hdc, 0, rcTitleBar.bottom, nullptr);
    LineTo(hdc, rcClient.right, rcTitleBar.bottom);
    SelectObject(hdc, oldPen);
    DeleteObject(pen);
}

// Function to create and draw the filter overlay
static HRESULT DrawFilterOverlay(HDC hdc, int width, int height, float animationProgress) {
    if (!hdc || width <= 0 || height <= 0) return E_FAIL;

    HDC memDC = CreateCompatibleDC(hdc);
    if (!memDC) return E_FAIL;

    // Create a fully transparent DIB section for alpha blending
    BITMAPINFO bmi = { 0 };
    bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    bmi.bmiHeader.biWidth = width;
    bmi.bmiHeader.biHeight = -height; // Top-down DIB
    ...
    if (pvBits) {
        memset(pvBits, 0, static_cast<size_t>(width) * static_cast<size_t>(height) * 4);
    }

    // Calculate alpha based on settings and animation
    BYTE alpha = static_cast<BYTE>(g_settings.opacity * 255.0f / 100.0f * animationProgress);

    // Calculate fade width in pixels
    const int fadePixels = (width * g_settings.fadeWidth) / 100;

    // Use GDI+ for high-quality drawing
    Graphics graphics(memDC);
    graphics.SetSmoothingMode(SmoothingModeHighQuality);
    graphics.SetCompositingQuality(CompositingQualityHighQuality);
    graphics.SetInterpolationMode(InterpolationModeHighQualityBicubic);

    // Fill center area with semi-transparent black
    if (alpha > 0) {
        SolidBrush centerBrush(Color(alpha, 0, 0, 0));
        graphics.FillRectangle(&centerBrush, fadePixels, 0, width - fadePixels * 2, height);
    }

    // Draw gradient fades on left and right sides if fade width > 0
    if (fadePixels > 0 && alpha > 0) {
        // Create left gradient with simple start/end colors
        ...
        leftBrush.SetGammaCorrection(TRUE);

        // Draw left gradient
        graphics.FillRectangle(&leftBrush, 0, 0, fadePixels, height);

        // Create right gradient with simple start/end colors
        LinearGradientBrush rightBrush(
            Point(width - fadePixels, 0),
            Point(width, 0),
            Color(alpha, 0, 0, 0),
            Color(0, 0, 0, 0)
        );
        rightBrush.SetGammaCorrection(TRUE);

        // Draw right gradient
        graphics.FillRectangle(&rightBrush, width - fadePixels, 0, fadePixels, height);
    }

    // Apply the buffer to the screen using UpdateLayeredWindow
    ...
    RECT rect;
    GetWindowRect(g_hFilterWindow, &rect);
    POINT ptDst = { rect.left, rect.top };

    // Update the layered window with the bitmap
    BOOL result = UpdateLayeredWindow(g_hFilterWindow, hdc, &ptDst, &size, memDC, &ptSrc, 0, &blend, ULW_ALPHA);
...
// The window procedure for the filter overlay window
static LRESULT CALLBACK FilterWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        if (g_animatingFilter) {
            animationProgress = static_cast<float>(g_currentAnimationStep) / ANIMATION_STEPS;
            if (!g_filterEnabled) {
                animationProgress = 1.0f - animationProgress;
            }
        }

        // Calculate alpha value based on opacity setting and animation
        BYTE alpha = static_cast<BYTE>(g_settings.opacity * 255.0 / 100.0 * animationProgress);
        ...
        bmi.bmiHeader.biHeight = -screenHeight; // Top-down bitmap
        bmi.bmiHeader.biPlanes = 1;
        ...
        if (hBitmap) {
            ...
            // Initialize the bitmap to transparent
            memset(pvBits, 0, static_cast<size_t>(screenWidth) * static_cast<size_t>(screenHeight) * 4);

            // Create GDI+ objects for drawing gradients
            Gdiplus::Graphics graphics(memDC);
            graphics.SetSmoothingMode(Gdiplus::SmoothingModeHighQuality);

            // Create a single, continuous gradient across the entire width
            // Calculate the center point and the position where the gradient starts/ends
            float centerX = screenWidth / 2.0f;
            float gradientWidth = (screenWidth * g_settings.fadeWidth) / 200.0f; // Half of 100% for each side

            // Create color array for the multi-stop gradient
            Gdiplus::Color colors[3] = {
                Gdiplus::Color(alpha, 0, 0, 0),  // Left edge - dark
                Gdiplus::Color(0, 0, 0, 0),      // Center - transparent
                Gdiplus::Color(alpha, 0, 0, 0)   // Right edge - dark
            };

            // Adjust positions based on fade width - larger fade width means wider transparent area
            float positions[3] = {
                0.0f,                       // Left edge is always 0
                0.5f,                       // Center is always 0.5
                1.0f                        // Right edge is always 1
            };

            // Create linear gradient brush
            Gdiplus::LinearGradientBrush brush(
                Gdiplus::Point(0, 0),
                Gdiplus::Point(screenWidth, 0),
            ...
            // Fill entire screen with the gradient - this creates the base effect
            graphics.FillRectangle(&brush, 0, 0, screenWidth, screenHeight);

            // If fade width is large enough, add a clear center area
            if (g_settings.fadeWidth > 10) {  // Only create a clear center if fade width is sufficient
                // Calculate width of center clear area
                int centerWidth = (screenWidth * (g_settings.fadeWidth - 10)) / 200;  // Scale from 0 to max width
                int centerStart = static_cast<int>(centerX - centerWidth / 2);

                if (centerWidth > 0) {
                    // Clear the center area by drawing a completely transparent rectangle
                    Gdiplus::SolidBrush clearBrush(Gdiplus::Color(0, 0, 0, 0));
                    graphics.FillRectangle(&clearBrush, centerStart, 0, centerWidth, screenHeight);
                }
            }
    ...
    // Handle display change for multi-monitor setups
